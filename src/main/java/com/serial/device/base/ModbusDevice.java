package com.serial.device.base;
...
public abstract class ModbusDevice {
     ...
     *
     * <p>
     * Delegates to {@link ModbusTransport#reconnect()}. Called by {@link com.serial.service.DeviceService} after a configurable
     * number of consecutive poll failures, which indicates the USB-to-serial adapter was physically disconnected and
     * reconnected.
     * </p>
     *
     ...
     *
     * @param reg the register descriptor containing address and scaling
     * @return the raw integer register value
     * @throws Exception if communication with the device fails
     */
    ...
    /**
     * Writes a value to a device register using a {@link DeviceRegister} descriptor and verifies the write by reading back both
     * the set register and the output register.
     *
     * @param regSet the register descriptor for the setpoint
     * @param regOut the register descriptor for the measured output
     * @param value  the engineering value to write
     * @throws Exception if communication fails or the value cannot be verified after {@link #MAX_RETRY} attempts
     */
    public void writeVerified(final DeviceRegister regSet, final DeviceRegister regOut, final double value) throws Exception {
        ...
        for (int attempt = 1; attempt <= MAX_RETRY; attempt++) {
            ...
            logger.info("  attempt {}: SET={} {} OUT={} {}", attempt, formatValue(readSet, regSet.scale), regSet.unit,
    ...
    /**
     * Writes a raw integer value to a device register using a {@link DeviceRegister} descriptor without any scaling.
     *
     * @param reg   the register descriptor
     ...
     *
     * <p>
     * Delegates to {@link ModbusTransport#readRegisters(byte, int, int)}, which sends one Modbus {@code 0x03} request for all
     * {@code count} registers and returns them in a single serial round-trip. The returned array is indexed by offset from
     * {@code startAddress}: element {@code [0]} is the value at {@code startAddress}, element {@code [1]} at
     * {@code startAddress + 1}, and so on.
     * </p>
     *
     * <p>
     * Values are raw (unscaled). Callers are responsible for applying the appropriate scale factor for each offset.
     * </p>
     *
     * <p>
     * Example:
     ...
     *
     * <p>
     * Delegates to {@link ModbusTransport#writeRegisters(byte, int, int[])}, which sends one Modbus {@code 0x10} request
     * covering all {@code values.length} registers starting at {@code startAddress}. This uses a single serial round-trip
     * regardless of how many registers are written.
     * </p>
     *
     * <p>
     * Values must already be scaled to raw register representation. Element {@code [0]} is written to {@code startAddress},
     * element {@code [1]} to {@code startAddress + 1}, and so on.
     * </p>
     *
     * <p>
